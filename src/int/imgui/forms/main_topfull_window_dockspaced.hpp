...
inline
ImGuiID MainTopfullWindowDockspaced()
{
	...
	if (opt_fullscreen)
	{
		...
		{
			const ImVec2 wcrMin = mainViewport->WorkPos;  // `wcr` - Window Content Region
			const ImVec2 wcrMax = mainViewport->WorkSize; // `wcr` - Window Content Region
			ImGui::GetForegroundDrawList()->AddRect(wcrMin, wcrMax, IM_COL32(255, 255, 0, 255)); // yellow - work region of full(screen) window

			const ImVec2 wcrNoTextBot(wcrMax.x, wcrMax.y - ImGui::GetTextLineHeight()); // dockspace region size (full  **minus text height**)
			ImGui::GetForegroundDrawList()->AddRect(wcrMin, wcrNoTextBot, IM_COL32_WHITE);

			const ImVec2 wcrNoTextSize = wcrNoTextBot - wcrMin; // window region size (full  **minus text height**)

			ImGui::SetNextWindowSize(wcrNoTextSize); // custom size
		}
	...
	{
		dockspaceID = ImGui::GetID("MainTopfullWindowDockspace");
