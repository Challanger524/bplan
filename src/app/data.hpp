...
private:
	inline static uidc_t tax_uidc = 0; // tax unique id counter
...
public:
	//~TaxUnit() {}
	TaxUnit() : id(++tax_uidc) {}
	TaxUnit(kind_t kind, std::string name, std::vector<TaxUnit> units = std::vector<TaxUnit>()) : id(tax_uidc++), is(kind), units(units), name(name) {
	...
	explicit operator const void *() const { return reinterpret_cast<const void *>(static_cast<intptr_t>(this->id)); }
