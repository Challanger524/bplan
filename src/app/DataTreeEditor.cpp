...
void TaxUnitEdit(TaxUnit &unit, short level = -1)
{
	...
	if (ImGui::TreeNodeEx(scast<const void *>(unit), sett::tree.tnFlags, "%s", scast<const char *>(unit)))
	{
...
void App::DataTreeEditor()
{
	...
	if (sett::tree.showFlagsCheckboxes) {
		im::CheckboxFlags("ImGuiTreeNodeFlags_OpenOnArrow"      , &sett::tree.tnFlags, ImGuiTreeNodeFlags_OpenOnArrow      );
		im::CheckboxFlags("ImGuiTreeNodeFlags_OpenOnDoubleClick", &sett::tree.tnFlags, ImGuiTreeNodeFlags_OpenOnDoubleClick);
		im::CheckboxFlags("ImGuiTreeNodeFlags_SpanAvailWidth"   , &sett::tree.tnFlags, ImGuiTreeNodeFlags_SpanAvailWidth   ); im::SameLine();	HelpMarker("Extend hit area to all available width instead of allowing more items to be laid out after the node.");
		im::CheckboxFlags("ImGuiTreeNodeFlags_SpanFullWidth"    , &sett::tree.tnFlags, ImGuiTreeNodeFlags_SpanFullWidth    );
		//im::CheckboxFlags("ImGuiTreeNodeFlags_SpanAllColumns"   , &sett::tree.tnFlags, ImGuiTreeNodeFlags_SpanAllColumns   ); im::SameLine();	HelpMarker("For use in Tables only.");
		im::CheckboxFlags("ImGuiTreeNodeFlags_Framed"           , &sett::tree.tnFlags, ImGuiTreeNodeFlags_Framed           );
		im::CheckboxFlags("ImGuiTreeNodeFlags_Selected"         , &sett::tree.tnFlags, ImGuiTreeNodeFlags_Selected         );
		im::CheckboxFlags("ImGuiTreeNodeFlags_Bullet"           , &sett::tree.tnFlags, ImGuiTreeNodeFlags_Bullet           );
		im::CheckboxFlags("ImGuiTreeNodeFlags_DefaultOpen"      , &sett::tree.tnFlags, ImGuiTreeNodeFlags_DefaultOpen      );
	...
	// test glyphs
	if (sett::edit.showDebugTextEncoding && im::TreeNode("DebugTextEncoding ##cpp"))
	{
