...
SqlServer::SqlServer()
{
	try {
		...
		conn->connect("Driver={ODBC Driver 17 for SQL Server};server=localhost;database=DemoDB;trusted_connection=Yes;");
		conn->setAutoCommit(false);

		//odbc::PreparedStatementRef psDrop   = conn->prepareStatement("DROP   TABLE IF EXISTS TAB")                  ; psDrop  ->executeQuery();
		//odbc::PreparedStatementRef psCreate = conn->prepareStatement("CREATE TABLE TAB (ID int, DATA varchar(255))"); psCreate->executeQuery();
		conn->prepareStatement("DROP   TABLE IF EXISTS TAB"                  )->executeQuery();
		conn->prepareStatement("CREATE TABLE TAB (ID int, DATA varchar(255))")->executeQuery();
		odbc::PreparedStatementRef psInsert = conn->prepareStatement("INSERT INTO  TAB (ID, DATA) VALUES (?, ?)");
		...
		//odbc::ResultSetRef rs = conn->prepareStatement("SELECT ID, DATA FROM TAB WHERE ID > 100")->executeQuery();
		odbc::PreparedStatementRef psSelect = conn->prepareStatement("SELECT ID, DATA FROM TAB WHERE ID > ?");
