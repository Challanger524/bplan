...
inline void SettingsUnloadLibconfig()
{
	...
	// Write out the updated configuration
	{
		try {
			...
			std::cerr << "Info: Updated configuration successfully written to: " << FNAME_CONFIG_SETTINGS_LIBCONFIG << std::endl;
		}
		catch (const lc::FileIOException &) {
			std::cerr << "I/O error while writing file: " << FNAME_CONFIG_SETTINGS_LIBCONFIG << std::endl;
			return;
		}
	}
}
